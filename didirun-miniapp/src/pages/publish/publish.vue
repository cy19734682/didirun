...
import { $get, post, requestSubscribeMessage } from "../../util/request.js";
  ...
  methods: {
    ...
    async calculation() {
      ...
      const result = await post("order/calculation", {
        startAddress:
          this.buyStyle === BUY_NEARBY && this.tabCurrent === HELP_BUY
            ? undefined
            : this.startAddress,
        endAddress: this.endAddress,
        weight: this.weights[this.weightCurrent].value,
        goodsDesc: this.desc,
        serviceType: this.tabCurrent,
      });
      uni.hideLoading();
      if (result.code === 200) {
    ...
    async publish() {
      const orderTmpId = this.notice.orderTempId;
