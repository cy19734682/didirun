<template>
  <view>
    ...
    <view v-if="byList.length > 0 && query.status === 'wait'">
      <view class="normal-tag"> 分配给我的订单 </view>
      <view class="p-30">
        <OrderItem
          ...
          :isRider="true"
          :options="item"
          @refresh="getToRiderList()"
        />
      </view>
    </view>
    <view>
      <view
        v-if="byList.length > 0 && query.status === 'wait'"
        class="normal-tag"
      >
        空闲订单
      </view>
    ...
    <view v-if="list.length === 0" class="text-center">
      <view class="iconfont icon-order fo-9 fo-80"> </view>
      <view class="fo-24 fo-9 mt-16"> 暂无订单 </view>
    </view>
    <view
      v-if="list.length > 0 && list.length === count"
      class="p-30 text-center fo-24 fo-9"
    >
      ---- 没有更多了 ----
    </view>
  </view>
