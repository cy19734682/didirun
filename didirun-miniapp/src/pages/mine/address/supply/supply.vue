<template>
  <view>
    <view class="p-30">
      <view class="fo-50"> 请补充地址信息 </view>
      <view class="flex flex-between item-center mt-32" @click="navToSearch">
        <view class="iconfont icon-location fo-32"></view>
        <view
          class="ml-32 fo-28 address-content flex flex-start item-center py-30"
        >
          <view class="address-content-text">
            <view v-if="!formData.province" class="fo-9"> 点击搜索地址 </view>
            <view class="fo-3 ell">
      ...
      <view class="py-30 border-b flex flex-start item-center">
        <input
          class="fo-28 address-input"
          style="margin-left: 64rpx; width: 626rpx"
          placeholder-class="fo-9"
          type="text"
          v-model="formData.addressDetail"
          placeholder="请补充具体门牌号码"
        />
      </view>
      ...
      <view class="py-30 border-b flex flex-start item-center">
        ...
        <input
          class="fo-28 address-input ml-32"
          style="width: 626rpx"
          placeholder-class="fo-9"
          type="number"
      ...
      <view class="py-30">
        <view class="fo-28 fo-3"> 智能填写 </view>
        <view class="auto-content mt-24">
          <textarea
            v-model="autoText"
            placeholder-class="fo-28 fo-9"
            class="fo-28 auto-text"
            placeholder="粘贴地址信息，自动拆分姓名、电话和地址"
          />
          <view
            :class="['btn-sm', autoText ? 'btn-sm--able' : 'btn-sm--disable']"
...
export default {
  ...
  computed: {
    ...mapGetters(["startAddress", "endAddress"]),
  },
  data() {
    return {
      formData: {
  ...
  onLoad(event) {
    ...
    if (this.type === "start" || this.type === "end") {
      const address =
        this.type === "start" ? this.startAddress : this.endAddress;
  ...
  methods: {
      ...
      if (result.code === 200) {
        ...
        if (result.data.address_components) {
          ...
          this.formData.streetNumber =
            result.data.address_components.street_number || "--";
    ...
    async submit() {
      ...
      if (this.type === "start") {
        this.setStartAddress(this.formData);
      }
      if (this.type === "end") {
        this.setEndAddress(this.formData);
      }
