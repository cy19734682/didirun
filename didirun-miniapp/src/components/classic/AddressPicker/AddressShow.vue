    ...
    <dd-card padding="30rpx" width="630rpx">
      ...
      <view class="show-item flex flex-between mt-8 item-center">
        ...
        <view v-if="start.province" class="address-content border-b py-30">
          <view class="fo-28 fo-3">
            {{ start.province }}{{ start.city }}{{ start.district
            }}{{ start.addressDetail }}
          </view>
          ...
          <view v-if="isRider">
            <view
              class="op-btn op-btn-info"
              @click="makePhoneCall(start.mobileNumber)"
            >
              <view class="iconfont icon-call fo-24 fo-w"> </view>
            </view>
            <view
              class="op-btn op-btn-green"
              @click="
                openLocation(
                  start.latitude,
                  start.longitude,
              ...
              "
            >
              <view class="iconfont icon-navigation fo-24 fo-w"> </view>
            </view>
          </view>
        </view>
        <view v-else class="address-content border-b py-30">
          <view class="fo-36"> 就近购买 </view>
        </view>
      </view>
      <view class="show-item flex flex-between item-center">
        ...
        <view class="address-content py-30">
          <view class="fo-28 fo-3">
            {{ end.province }}{{ end.city }}{{ end.district
            }}{{ end.addressDetail }}
          </view>
          <view class="fo-24 fo-6">
        ...
        <view v-if="isRider">
          <view
            class="op-btn op-btn-info"
            @click="makePhoneCall(end.mobileNumber)"
          >
            <view class="iconfont icon-call fo-24 fo-w"> </view>
          </view>
          <view
            class="op-btn op-btn-green"
            @click="
              openLocation(end.latitude, end.longitude, end.addressDetail)
            "
          >
            <view class="iconfont icon-navigation fo-24 fo-w"> </view>
          </view>
