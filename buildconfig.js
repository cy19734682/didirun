    ...
    connection.query(
      ...
      (error) => {
        if (error) {
          if (error.errno === 1007) {
            console.error(
              `【创建数据库】- 失败 数据库名'${config.mysql.database}'已存在,可以直接使用`
            );
          } else {
...
const initConfig = () => {
  ...
  const read = fs.readFileSync(
    path.join(__dirname, "didirun-server", "src", "config", "config.prod.ts.bak"),
    {
      encoding: "utf8",
    }
  );
  const wirteStr = read
    ...
    .replace("{real_pay}", config.realPay);

  fs.writeFileSync(
    path.join(__dirname, "didirun-server", "src", "config", "config.prod.ts"),
    wirteStr
  );
...
const execRrocess = (script, label) => {
  console.info(`【${label}】......`);
  return new Promise((resolve) => {
    const spawn = child_process.spawn("pnpm", [script], { shell: true });
...
const initAdminConfig = () => {
  console.info("【后台配置文件】......");
  const read = fs.readFileSync(
    path.join(__dirname, "didirun-admin", "nuxt.config.js.bak"),
    {
      encoding: "utf8",
    }
  );
  ...
  fs.writeFileSync(path.join(__dirname, "didirun-miniapp", "nuxt.config.js"), wirteStr);
...
const initMiniappConfig = () => {
  ...
  const read = fs.readFileSync(
    path.join(__dirname, "didirun-miniapp", "pages.json.bak"),
    {
      encoding: "utf8",
    }
  );
  ...
  fs.writeFileSync(path.join(__dirname, "didirun-miniapp", "pages.json"), wirteStr);

  const read2 = fs.readFileSync(
    path.join(__dirname, "didirun-miniapp", "util", "constant.js.bak"),
    {
      encoding: "utf8",
    }
  );
  ...
  fs.writeFileSync(
    path.join(__dirname, "didirun-miniapp", "util", "constant.js"),
    wirteStr2
  );
