...
export class OrderService extends BaseService {
  ...
  getTableName() {
    return this.orderEntity.metadata.tableName;
  }
  /**
   * 取消订单
   * @param dto
  ...
  async add(
    ...
    const result = await this.orderEntity.manager.transaction(async () => {
      ...
      if (status === 1) {
        ...
        if (order) {
          this.cityService.sendMessageToChat(order);
        }
      }
      return orderNo;
    });
  ...
  async calculationPrice({
    ...
    if (city.citysValuationId) {
      ...
      for (const dtans of rule.distance) {
        ...
        // 若距离覆盖此范围
        else if (distance > dtans.lte * 1000) {
          if (dtans.unitDistance > 0) {
            distancePrice +=
              Math.ceil((dtans.lte - dtans.gt) / dtans.unitDistance) *
              dtans.price;
          } else {
            distancePrice += dtans.price;
          }
      ...
      for (const wet of rule.weight) {
        ...
        } else if (weightValue > wet.lte) {
          if (wet.unitWeight > 0) {
            weightPrice +=
              Math.ceil((wet.lte - wet.gt) / wet.unitWeight) * wet.price;
          } else {
            weightPrice += wet.price;
          }
        }
   ...
   * @param orderNo
   */
  async autoPay(orderNo: string) {
    const order = await this.findByNo(orderNo);
    if (order.status === 0) {
      ...
      // 群发消息
      await this.cityService.sendMessageToChat(order);
    }
  }
